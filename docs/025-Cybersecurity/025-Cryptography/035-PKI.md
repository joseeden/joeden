...
| **Serial Number**                  | Unique number issued by the certificate creator.           |
| **Signature Algorithm Identifier** | Method used by the CA to sign the certificate.             |
| **Issuer Name**                    | Information about the Certificate Authority (CA).          |
| **Validity Period**                | Expiration dates of the certificate.                       |
| **Subject Name**                   | Common Name (CN) and Distinguished Name (DN) of the owner. |
| **Subject's Public Key**           | Public key belonging to the certificate owner.             |
