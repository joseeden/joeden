...
In JavaScript, [type coercion](/docs/065-Software-Engineering/012-Javascript/002-Fundamentals/012-Strings.md#type-coercion) refers to the automatic or implicit conversion of values from one data type to another. JavaScript will attempt to convert one or both of the values to make the operation possible.
...
Be cautious when checking if something is "defined" because falsy values like `0` might cause issues. 

Here, `height` is `undefined` (considered a falsy value) so the else block runs.
