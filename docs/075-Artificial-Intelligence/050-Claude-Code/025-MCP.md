...
![](/gif/docs/06062026-claude-code-mcp-demo.gif)
...
You can connect external tools and data sources by adding Model Context Protocol servers using a simple terminal command. The system supports two primary types of server connections depending on where the service is hosted.

- **HTTP servers** - Connect remote services using HTTP servers
- **Stdio servers** - Run local processes using Stdio servers

Sample command to add an HTTP server:
...
You can configure your server connections to be available at different levels depending on your project needs. 

| Scope       | Description                                                      |
| ----------- | ---------------------------------------------------------------- |
| **Local**   | Makes the server available only in the current project for you.  |
| **User**    | Makes the server available across all your personal projects.    |
| **Project** | Shares the server setup with everyone working on the repository. |

Sample configuration file showing a local project scope for an SQLite database server:
...
Here, the `mcpServers` configuration restricts the database tool to this specific folder. Setting the correct scope helps protect your global environment from project-specific configurations.

**NOTE:** MCP servers add tools to your context even when they're not being used. Too many servers can reduce the available context for your conversations. 

To review the connected servers, check the server list.
